...
from mysql_arguments import username, password, port, database, host

dialect = "mysql"
driver = "pymysql"
address = dialect + "+" + driver + "://" + username + ":" + password + "@" + host + ":" + port + "/" + database
...
def get_from_db(symbol, start_date, end_date):
    ...
    with sqlEngine.begin() as connection:
        series = pd.read_sql("SELECT * FROM " + symbol + " WHERE date >= '" + start_date + "' AND date <= '" + end_date + "'",
...
def dates_in_table(symbol, start_date, end_date, connection):
    ...
    series = get_from_db(symbol, start_date, end_date)  # There is perhaps redundancy here as we open engine in get_from_db too. 
    if len(series) >= 1:
        return True
    return False